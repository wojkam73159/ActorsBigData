...
with DAG(
  ...
  mapreduce_classic = BashOperator(
    ...
    bash_command="""hadoop jar {{ params.dags_home }}/project_files/transform2.jar CompositeValueJob {{ params.input_dir }}/datasource1 {{ params.output_mr_dir }}""",
  )
...
... """,
  ...
  pig = BashOperator(
    task_id="pig",
    bash_command="""export PIG_CLASSPATH=/etc/hadoop/conf.empty:/etc/tez/conf; pig -x tez -f {{ params.dags_home }}/project_files/transform5.pig \
